...
public class VolumeControl extends View {
    ...
    private int maxLineHeight = 30;
    private int maxSpaceBetweenLines = 10;
    ...
    public VolumeControl(Context context, @Nullable AttributeSet attrs) {
        ...
        try{
            volumeScale = xmlProperties.getInt(R.styleable.VolumeControl_volumeScale, 10);
            volumeLevelPercentage = xmlProperties.getInt(R.styleable.VolumeControl_volumeLevelPercentage, 50);
            linesColor = xmlProperties.getColor(R.styleable.VolumeControl_linesColor, Color.BLUE);
        }finally {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(getResources().getString(R.string.volume_is_set_at,this.volumeLevelPercentage) + "%", getWidth()/2, volumeScale*combinedPartHeight + textSize + spaceBetweenTextAndLines, mTextPaint);
    }
}
